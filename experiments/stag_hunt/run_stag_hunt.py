"""
stag_hunt_groq_v1 — 2-player iterated Stag Hunt on Groq.

English symbolic prompts (new experiment_id — see .cursorrules). Referee is
LLM-free (payoff matrix lookup + running score + stag rate / coordination /
early-late stag-rate metrics). Runs are sequential for now (no cross-run
concurrency); within a round, both agents decide in parallel via
asyncio.gather.

Trait dimensions (see scenarios/stag_hunt/agents.py docstring for the full
interpretive rationale):
  - risk_tolerance: 0=always hare (safe), 1=always stag (risky coordination)
  - cooperation: trust/expectation that the partner will choose stag

Usage (from repo root):
    python experiments/stag_hunt/run_stag_hunt.py --plan
    python experiments/stag_hunt/run_stag_hunt.py --plan --micro-pilot
    python experiments/stag_hunt/run_stag_hunt.py --micro-pilot         # 10 runs (API), coop trait
    python experiments/stag_hunt/run_stag_hunt.py --risk-micro-pilot    # 10 runs (API), risk trait
    python experiments/stag_hunt/run_stag_hunt.py                       # full grid (API)
...
from scenarios.stag_hunt.agents import (
    _INPUT_COST_PER_M,
    _OUTPUT_COST_PER_M,
    reset_token_usage,
    token_usage,
...
from scenarios.stag_hunt.persistence import (
    STAG_HUNT_DB_PATH,
    init_stag_hunt_db,
    register_stag_hunt_conditions,
)
from scenarios.stag_hunt.state import AGENT_A_ID, AGENT_B_ID, StagHuntState

EXPERIMENT_ID = "stag_hunt_groq_v1"
MAX_ROUNDS = 12
REPLICATIONS = 5
...
# Coop micro-pilot (run first): cooperation ∈ {0.2, 0.8}, risk fixed at 0.5 → 10 runs.
MICRO_COOP_LEVELS = ("low", "high")
MICRO_RISK_LEVEL = "medium"

# Risk micro-pilot: risk ∈ {0.2, 0.8}, cooperation fixed at 0.5 → 10 runs.
...
def _make_initial_state(spec: RunSpec, experiment_id: str) -> StagHuntState:
...
def _register_plan(specs: list[RunSpec], experiment_id: str) -> None:
    ...
    register_stag_hunt_conditions(experiment_id, rows, STAG_HUNT_DB_PATH)
...
def _print_plan(
    ...
    *,
    ...
    print("=" * 72)
    print(f"STAG HUNT — EXPERIMENT PLAN ({mode}, dry run / not executed)")
    print("=" * 72)
    print(f"  experiment_id          : {experiment_id}")
    print(f"  game                   : iterated stag hunt (2-player, simultaneous)")
    print(f"  database               : {STAG_HUNT_DB_PATH}")
    print(f"  prompt style           : English symbolic")
    print(f"  total runs             : {len(specs)}")
    print(f"  replications/condition : {REPLICATIONS}")
    print(f"  max_rounds             : {MAX_ROUNDS}")
    print(f"  payoffs (SS,HH,HS,SH)  : (5,5), (3,3), (4,0), (0,4)")
...
def _print_final_summary(
    ...
    init_stag_hunt_db(STAG_HUNT_DB_PATH)
    with sqlite3.connect(STAG_HUNT_DB_PATH) as conn:
        rounds = conn.execute(
            "SELECT COUNT(*) FROM stag_hunt_rounds WHERE experiment_id = ?",
            (experiment_id,),
        ).fetchone()[0]
        metrics = conn.execute(
            "SELECT COUNT(*) FROM stag_hunt_metrics WHERE experiment_id = ?",
            (experiment_id,),
        ).fetchone()[0]
        conditions = conn.execute(
            "SELECT COUNT(*) FROM stag_hunt_conditions WHERE experiment_id = ?",
            (experiment_id,),
        ).fetchone()[0]
        summary_rows = conn.execute(
            "SELECT COUNT(*) FROM stag_hunt_summary WHERE experiment_id = ?",
            (experiment_id,),
        ).fetchone()[0]
    print(f"  DB — stag_hunt_rounds  : {rounds}")
    print(f"  DB — stag_hunt_metrics : {metrics}")
    print(f"  DB — stag_hunt_conditions: {conditions}")
    print(f"  DB — stag_hunt_summary : {summary_rows}")
...
async def run_experiment(
    *,
    ...
    if risk_micro_pilot:
        ...
        mode = "FULL"
    print("=" * 72)
    print(f"STAG HUNT — {mode} STARTING (sequential runs)")
    print("=" * 72)
    print(f"  experiment_id : {experiment_id}")
    print(f"  total runs    : {len(specs)}")
    ...
    print(f"\n  Wrote/updated {len(specs)} rows in stag_hunt_conditions.")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Stag Hunt runner (coop micro / risk micro / full).",
    )
    parser.add_argument(
        "--plan",
        action="store_true",
    ...
    parser.add_argument(
        "--micro-pilot",
        action="store_true",
        help="coop∈{0.2,0.8}, risk=0.5, 5 reps/cell = 10 runs (run this one first)",
    )
    parser.add_argument(
        "--risk-micro-pilot",
        action="store_true",
